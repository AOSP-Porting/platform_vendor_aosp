...
func init() {
	android.RegisterModuleType("extra_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = extraExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = extraExpandVariables(ctx, dep_file)
	...
	cmd := extraExpandVariables(ctx, String(g.properties.Cmd))
